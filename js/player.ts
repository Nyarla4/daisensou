import { CreatureData, PlayerState, StageData } from "./interfaces.js";

/** 현재 플레이어 상태 */
export let playerState: PlayerState;

/** json 플레이어 데이터 로드 */
async function fetchPlayerData(): Promise<PlayerState> {
...
/** 사용 가능한 개체 여부 */
export function canUseCreature(creature: CreatureData): boolean {
    return getCreatureLevel(creature.id) >= 1;
}

/** 개체 레벨 조회 */
export function getCreatureLevel(creatureId: string): number {
    return playerState.creatureLevels[creatureId] ?? 0;
}

/** 스테이지 클리어 보상 처리 */
export function rewardStageClear(stageData: StageData) {
